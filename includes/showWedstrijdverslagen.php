...
if (isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($reports->count()) {
        foreach ($reports->results() as $report) {
            ...
            if ($user->isLoggedIn() && ($user->hasPermission('dev') || $user->data()->id == $report->user_id || $user->hasPermission("reportremove"))) {
                echo '<i title="Verijderen" class="fa fa-trash-o" style="float: right;" onclick="removeReport(' . escape($report->id) . ')"></i>';
            } if ($user->isLoggedIn() && ($user->hasPermission('dev') || $user->data()->id == $report->user_id || $user->hasPermission("reportedit"))){
