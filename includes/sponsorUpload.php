...
if ($user->isLoggedIn() && ($user->hasPermission('dev') || $user->hasPermission('sponsorupload'))) {
    ...
    if (!empty(trim($_POST['name'])) && isset($_POST['name']) && !empty(trim($_POST['title'])) && isset($_POST['title']) && !empty(trim($_POST['link'])) && isset($_POST['link'])) {
        ...
        if (preg_match("#^[a-zA-Z0-9 '!' '?' ',' '.' ' ' '(' ')' ':' '\-' '_' '=' '*' '\'' '\"' '%']+$#", $title)) {
            if (substr($link, 0, 6) === 'http://' | substr($link, 0, 7) === 'https://') {
                ...
                $link = null;
                echo "<h3>Voer een geldige link in</h3><br>";
                echo "Een link moet beginnen met http:// of https://<br><br>";
            }
        } else {
            $title = null;
            echo "<h3>Voer een geldige titel in</h3><br>";
            echo "Karakters die u kunt gebruiken zijn: a-z A-Z 0-9 . , ? ! ( ) : = - _ * ' \" %<br><br>";
        }
    } else {
        echo "<h3>Voer een geldig bericht in</h3><br>";
